function updateUserPassword(token, currentPassword, newPassword, newPasswordRepeat, callback) {
    if (typeof token !== 'string') throw new TypeError('token is not a string')
    if (token.trim().length === 0) throw new Error('token is empty or blank')

    if (typeof currentPassword !== 'string') throw new TypeError('old password is not a string')
    if (currentPassword.trim().length === 0) throw new Error('old password is empty or blank')
    if (currentPassword.length < 8) throw new Error('old password length is less than 8 characters')
    ...
    const json = JSON.stringify({ oldPassword: currentPassword, password: newPassword })
