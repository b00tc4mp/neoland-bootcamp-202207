class Home extends Component {
  constructor() {
    ...
    const closeButton = templateToDOM(
      '<button class="close-button transparent-button"><span class="material-symbols-outlined">close</span></button>'
    );
    ...
    menuButton.onclick = () => {
      ...
      if (main.contains(settingsPanel.container)) menuPanel.hideSettings();

      header.append(menuPanel.container);
    };

    closeButton.onclick = () => {
      if (headerTop.contains(closeButton)) headerTop.removeChild(closeButton);
