...
const { DuplicityError, SystemError, UnknownError } = require("../errors");
const {
  validateText,
  validateEmail,
  validatePassword,
...
function registerUser(name, email, password, callback) {
  ...
  readdir(folder, (error, files) => {
    ...
    if (error)
      return callback(
        new SystemError(`cannot list files from folder ${folder}`)
      );
    ...
    if (files.length) {
      ...
      (function iterate() {
        readFile(`./data/users/${file}`, "utf8", (error, json) => {
          if (error)
            return callback(
              new SystemError(`cannot read file ${file} in folder ${folder}`)
            );
          ...
          if (user.email === email)
            return callback(
              new DuplicityError(`user with email ${email} already exists`)
            );
