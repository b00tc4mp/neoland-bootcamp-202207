...
describe("concat", function () {
  it("concatenates two arrays with numbers", function () {
  ...
  it("concatenates two cachays with strings", function () {
    const cachay1 = ["h", "o", "l", "a"];
    const cachay2 = ["m", "u", "n", "d", "o"];
    const result = cachay1.concat(cachay2);
  ...
  it("concatenates three cachays with numbers", function () {
    const cachay1 = new Cachay(1, 2);
    ...
    expect(result.length).to.equal(
      cachay1.length + cachay2.length + cachay3.length
    );
    expect(result[0]).to.equal(cachay1[0]);
    expect(result[1]).to.equal(cachay1[1]);
    expect(result[2]).to.equal(cachay2[0]);
  ...
  it("concatenates four cachays with numbers", function () {
    const cachay1 = new Cachay(1, 2);
    const cachay2 = new Cachay(3, 4);
    const cachay3 = new Cachay(5, 6);
    ...
    expect(result.length).to.equal(
      cachay1.length + cachay2.length + cachay3.length + cachay4.length
    );
    expect(result[0]).to.equal(cachay1[0]);
    expect(result[1]).to.equal(cachay1[1]);
    expect(result[2]).to.equal(cachay2[0]);
  ...
  it("concatenates five cachays with numbers", function () {
    const cachay1 = new Cachay(1, 2);
    const cachay2 = new Cachay(3, 4);
    const cachay3 = new Cachay(5, 6);
    const cachay4 = new Cachay(7, 8);
