...
function NoteList({ notes, onDeleteNote, onUpdateNote }) {
  ...
  useEffect(() => {
    ...
    if (notes) {
      notes.map((note) => textAreaAdjust(note._id));
  ...
  return (
    <ul className="list-panel list">
      {notes &&
        notes.map((note) => (
          <li className="list__item" key={note._id}>
            <button
            ...
            <textarea
              ref={(ref) => (noteText[note._id] = ref)}
              className="list__item-text"
              onKeyUp={(event) => {
                textAreaAdjust(note._id);
