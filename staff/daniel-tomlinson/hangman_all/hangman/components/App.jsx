class App extends React.Component {
  ...
  handleOnChooseWordFormSubmit = (event) => {
    ...
    this.setState({
      result: wordSelected,
      view: "playing",
      wordShown: "_".repeat(wordSelected.length),
    });
  };
  ...
  handleOnCharacterForm = (event) => {
    ...
    if (this.state.result.includes(charTried)) {
      ...
      for (let i = 0; i < this.state.result.length; i++)
        if (this.state.result[i] === charTried) newWordShown[i] = charTried;
  ...
  handleWinOrLose = (newWordShown) => {
    ...
    if (newWordShown.join("") === this.state.result)
      this.setState({ view: "winner" });
  };
        ...
        {this.state.view === "select word" && (
          <div className="container">
            ...
            <Form
              ...
              onSubmit={this.handleOnChooseWordFormSubmit}
              buttonText="START"
            />
          </div>
        )}
        ...
        {this.state.view === "playing" && (
          <div className="container">
            ...
            <Form
              ...
              buttonText="TRY"
              onSubmit={this.handleOnCharacterForm}
            />
          </div>
        )}
